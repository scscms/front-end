...
ECMAScript中的函数是对象，因此函数也有属性和方法。每个函数都包含两个属性：length和prototype，其中，length属性表示函数希望接收到饿命名参数的个数。
...
每个函数继承的toLocalStirng()和toString()方法始终都返回函数的代码。另一个继承的valueOf()方法同源也只返回函数代码。
